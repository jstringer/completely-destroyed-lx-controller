...
- ⏭ **Deferred:** C5 Fork (deep-copy patch); patch-editor playhead-preview curve + Source/Modulation zone restyle (editor still uses the working flat layout); Pad/Knob kind chip (not in model); named per-claim Output columns (needs a service accessor; shipped honest dimmer-level + claim-count instead). Phase F (OSC) / Phase G (polyphony) remain future.
