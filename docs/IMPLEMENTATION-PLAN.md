...
- ⏭ **Deferred (ponytail):** **B1** view-module split and **B3 widget helpers** (SectionHeader/DangerButton/DisabledRegion/LabeledCombo/ModPlot) fold into **Phase E** — E rewrites each surface, so a pure no-op split + helpers-without-callers ahead of E is scaffolding-for-later. The Custom luminous palette (vs HyperDark) is also deferred: its `RGBColor8` JSON shape is unverifiable from the SDK (no source/sample config), so accents live in `lxtheme.h` per the plan's own fallback.
- ⏭ **Next:** Phase E (the UI, with C3/C4/C5 + B1 + widget helpers folded in). The largest chunk.
